...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ── 광고 진단 (게시 실패 원인: 검수 이슈/유효상태) ──
    if (action === "get_ad") {
      const id = req.query.id;
      if (!id) return res.status(400).json({ error: "id required" });
      const r = await fetch(`${META_BASE}/${id}?fields=id,name,status,effective_status,issues_info,recommendations,adset{id,name,daily_budget,optimization_goal,promoted_object,effective_status,issues_info},creative{id,instagram_user_id,object_story_spec}&access_token=${META_TOKEN}`);
      const d = await r.json();
      if (d.error) return res.status(400).json({ error: fbErr(d.error) });
      return res.status(200).json(d);
    }
