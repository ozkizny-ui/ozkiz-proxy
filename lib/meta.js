...
async function createAd(body, ctx) {
  ...
  // 인스타 계정 고정(ozkiz_official). 페이지 연결에만 의존하면 IG란이 비는 경우가 있어 명시 핀.
  const IG_USER_ID = ctx.IG_USER_ID || process.env.META_IG_USER_ID || "17841402167601732";
  const {
  ...
  if (ad_type === "CAROUSEL" && Array.isArray(cards) && cards.length >= 2) {
    ...
    creativeBody.object_story_spec = {
      ...
      link_data: {
        ...
        child_attachments: cards.map((c) => {
          ...
          const att = { image_hash: c.image_hash, link: cl, call_to_action: { type: "SHOP_NOW", value: { link: cl } } };
          if (c.name) att.name = c.name; // 카드 헤드라인 있을 때만 (없으면 name 생략 → 광고세트명 안 들어감)
          return att;
        }),
      },
    };
    ...
    // 컬렉션: 실제 작동 광고(creative) 역설계 구조 —
    //   link_data { image_hash(커버) + retailer_item_ids 4칸(카탈로그 자동 타일) } + product_set_id.
    // (기존 버그: 빈 child_attachments[] → #194 / template_data+image_hash → 이미지거부.
    //  진짜 컬렉션은 retailer_item_ids를 쓰고, 커버 image_hash는 link_data에서 허용됨.)
    // ※ 작동 광고는 CTA에 인스턴트경험(canvas_doc) 링크도 있었음 → 우선 canvas 없이 Meta 자동생성 시도. 실패 시 canvas 생성 추가.
    const linkData = {
      link: urlWithUtm,
      message: caption,
      call_to_action: { type: "SHOP_NOW", value: { link: urlWithUtm } },
      retailer_item_ids: ["0", "0", "0", "0"], // 컬렉션 제품 타일(카탈로그에서 자동 채움)
    };
    if (image_hash) linkData.image_hash = image_hash;          // 커버 이미지(사용자 업로드) — link_data에선 허용
    if (collection_label) linkData.name = collection_label;    // 컬렉션 제목(선택)
    creativeBody.object_story_spec = { page_id: PAGE_ID, link_data: linkData };
  ...
  // 인스타 신원 고정 — ozkiz_official. (PA는 게시물 자체의 IG를 쓰므로 핀 안 함)
  if (ad_type !== "PA" && IG_USER_ID) creativeBody.instagram_user_id = IG_USER_ID;
