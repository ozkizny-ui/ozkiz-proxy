...
async function createAd(body, ctx) {
  ...
  if (ad_type === "CAROUSEL" && Array.isArray(cards) && cards.length >= 2) {
    ...
    // 카탈로그 컬렉션: link_data+수동 child_attachments([]) → template_data(커버) + product_set_id.
    // 컬렉션 그리드(타일)는 Meta가 product_set에서 자동 생성. 기존 빈 child_attachments가 #194 유발하던 버그 수정.
    creativeBody.object_story_spec = {
      page_id: PAGE_ID,
      template_data: {
        link: urlWithUtm,
        ...
        ...(collection_label ? { name: collection_label } : {}), // 컬렉션 제목(선택)
      },
      // ⚠️ 컬렉션/다이내믹은 카탈로그(product_set) 이미지를 사용 → 수동 image_hash 금지(Meta subcode 2446380). 그래서 image_hash 안 보냄.
    };
