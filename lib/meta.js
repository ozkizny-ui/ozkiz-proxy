...
async function createAd(body, ctx) {
  ...
  const {
    ...
    image_hash, video_id, product_set_id, collection_label, post_url, cards,
  } = body || {};
  ...
  if (ad_type === "CAROUSEL" && Array.isArray(cards) && cards.length >= 2) {
    ...
    creativeBody.object_story_spec = {
      page_id: PAGE_ID,
      video_data: { video_id, call_to_action: { type: "SHOP_NOW", value: { link: urlWithUtm } }, message: caption },
    };
...
// 영상 처리상태 조회 (create_ad 전 ready 확인용). 반환 status_processing|ready|error 등
...
module.exports = { META_BASE, fbErr, createAd, uploadImage, uploadVideoByUrl, getVideoStatus };
