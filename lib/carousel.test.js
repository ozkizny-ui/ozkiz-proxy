...
function check(name, rows, expect) {
  ...
  if (expect.ok) {
    ok = r.ok && r.adName === expect.adName
      ...
      && (!expect.headlines || JSON.stringify(r.cards.map(c => c.headline)) === JSON.stringify(expect.headlines));
  } else {
...
check("정상(행 뒤섞임 3장) + 카드헤드라인 번호순", [
  { filename: `${C} (2).png`, card_landing_url: "https://ozkiz.com/p/2", card_headline: "2번 제목" },
  { filename: `${C} (1).png`, card_landing_url: "https://ozkiz.com/p/1", campaign: "여름캠페인", caption: "신상 모음전", card_headline: "1번 제목" },
  { filename: `${C} (3).png`, card_landing_url: "https://ozkiz.com/p/3", card_headline: "" },
], { ok: true, adName: C, order: [1, 2, 3], urls: ["https://ozkiz.com/p/1", "https://ozkiz.com/p/2", "https://ozkiz.com/p/3"], caption: "신상 모음전", campaign: "여름캠페인", headlines: ["1번 제목", "2번 제목", ""] });
