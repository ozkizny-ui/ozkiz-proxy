...
async function sbPost(key, table, rows) {
  const r = await fetch(`${SB_URL}/rest/v1/${table}`, { method: "POST", headers: sbHeaders(key, { Prefer: "return=minimal" }), body: JSON.stringify(rows) });
  if (!r.ok) throw new Error(`sb insert ${table} ${r.status}: ${await r.text()}`);
}

// 입찰 변경 이력 기록 (log_bid_change). rows:[{channel,entity_id,name,old_bid,new_bid}]
export async function logBidChanges(env, rows) {
  const key = env.SUPABASE_SERVICE_ROLE_KEY;
  if (!key) { const e = new Error("SUPABASE_SERVICE_ROLE_KEY not configured"); e.status = 500; throw e; }
  if (!Array.isArray(rows) || !rows.length) { const e = new Error("rows[] required"); e.status = 400; throw e; }
  const clean = rows.slice(0, 500).map(r => ({
    channel: r.channel === "powerlink" ? "powerlink" : "shopping",
    entity_id: String(r.entity_id || ""),
    name: r.name != null ? String(r.name).slice(0, 200) : null,
    old_bid: Number.isFinite(+r.old_bid) ? Math.round(+r.old_bid) : null,
    new_bid: Number.isFinite(+r.new_bid) ? Math.round(+r.new_bid) : null,
  })).filter(r => r.entity_id);
  if (!clean.length) { const e = new Error("no valid rows"); e.status = 400; throw e; }
  await sbPost(key, "nv_bid_changes", clean);
  return { ok: true, logged: clean.length };
}
// 입찰 변경 이력 조회 (get_bid_changes). channel 필터 + 최근순 limit.
export async function readBidChanges(env, channel, limit) {
  const key = env.SUPABASE_SERVICE_ROLE_KEY;
  if (!key) { const e = new Error("SUPABASE_SERVICE_ROLE_KEY not configured"); e.status = 500; throw e; }
  const lim = Math.min(Math.max(parseInt(limit, 10) || 30, 1), 200);
  const ch = channel === "powerlink" || channel === "shopping" ? `&channel=eq.${channel}` : "";
  const rows = await sbGet(key, "nv_bid_changes", `select=changed_at,channel,entity_id,name,old_bid,new_bid&order=changed_at.desc&limit=${lim}${ch}`);
  return { changes: rows };
}
...
// 수집 현황 조회(프론트 '수집·알림 현황' 탭용). 읽기전용. action=collect_status가 호출.
export async function readCollectStatus(env) {
  const key = env.SUPABASE_SERVICE_ROLE_KEY;
  if (!key) { const e = new Error("SUPABASE_SERVICE_ROLE_KEY not configured"); e.status = 500; throw e; }

  // 최근 intraday 실행 시각·건수(6시간마다 잘 도는지) — 한 실행은 captured_at 동일(단일 트랜잭션)
  const intraday = await sbGet(key, "nv_stat_snapshots", "select=captured_at&kind=eq.intraday&order=captured_at.desc&limit=500");
  const runMap = {};
  intraday.forEach(r => { runMap[r.captured_at] = (runMap[r.captured_at] || 0) + 1; });
  const runs = Object.entries(runMap).map(([at, ads]) => ({ at, ads })).sort((a, b) => b.at.localeCompare(a.at)).slice(0, 10);

  // 일별 완결(ROAS 프로파일 형성 확인) — stat_dt별 합산
  const dailyRows = await sbGet(key, "nv_stat_snapshots", "select=stat_dt,cost,conv_cnt,conv_val&kind=eq.daily&order=stat_dt.desc&limit=1000");
  const dmap = {};
  dailyRows.forEach(r => { const d = (dmap[r.stat_dt] ||= { stat_dt: r.stat_dt, cost: 0, convVal: 0, convCnt: 0 }); d.cost += Number(r.cost) || 0; d.convVal += Number(r.conv_val) || 0; d.convCnt += Number(r.conv_cnt) || 0; });
  const daily = Object.values(dmap).map(d => ({ ...d, roas: d.cost ? Math.round(d.convVal / d.cost * 100) : null })).sort((a, b) => a.stat_dt.localeCompare(b.stat_dt)).slice(-14);

  // 최근 알림 이력
  const alerts = await sbGet(key, "nv_alert_log", "select=created_at,kind,ref,detail&order=created_at.desc&limit=20");

  return { lastRun: runs[0] || null, runs, daily, alerts };
}
